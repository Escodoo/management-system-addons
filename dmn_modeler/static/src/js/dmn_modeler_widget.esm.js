...
// eslint-disable-next-line sort-imports
import {DMNModeler} from "./dmn_modeler.esm";

export class DmnModelerWidget extends Component {
    static template = xml`
        <div class="o_field_dmn_modeler" t-ref="container">
            <div class="o_dmn_drop_zone" t-if="state.showDropZone">
                <div class="o_dmn_drop_overlay">
                    <i class="fa fa-cloud-upload fa-3x"/>
                    <p>Drop DMN file here to import</p>
                </div>
            </div>
            <div class="o_dmn_toolbar">
                <div class="o_dmn_toolbar_group">
                    <button
                        class="btn btn-secondary"
                        t-on-click="onImportClick"
                        t-att-disabled="isImportDisabled"
                        title="Import DMN file"
                    >
                        <i class="fa fa-upload"/> Import
                    </button>
                    <button class="btn btn-primary" t-on-click="onExportClick" title="Export as DMN XML">
            ...
            <div class="o_dmn_validation_stats" t-if="state.validationStats">
                <small>
                    <i class="fa fa-info-circle"/>
                    Elements: <t t-esc="state.validationStats.totalElements"/> |
                    Decisions: <t t-esc="state.validationStats.decisions"/> |
                    Tables: <t t-esc="state.validationStats.decisionTables"/> |
                    Inputs: <t t-esc="state.validationStats.inputs"/> |
                    Outputs: <t t-esc="state.validationStats.outputs"/> |
                    Rules: <t t-esc="state.validationStats.rules"/>
                </small>
            </div>
            <DMNModeler
                dmn_xml="props.value || ''"
                ...
                onReady="(api) => this._onDmnReady(api)"
    ...
    /**
     * Handles the change event from the OWL DMNModeler component.
     * @param {String} xmlContent The updated DMN XML string.
     * @private
     */
    ...
    /**
     * Handles the SVG change event from the OWL DMNModeler component.
    ...
    /**
     * Called when DMN modeler is ready and exposes its API.
        ...
        const handleDrop = async (e) => {
            ...
            if (files && files.length > 0) {
                ...
                if (this._validateFile(file)) {
                    ...
                    this.notification.add(
                        this.env._t("Invalid file. Please drop a .dmn or .xml file."),
                        {type: "danger"}
                    );
                }
            }
    ...
    _validateFile(file) {
        ...
        const validExtensions = [".dmn", ".xml"];
        const fileName = file.name.toLowerCase();
        const hasValidExtension = validExtensions.some((ext) => fileName.endsWith(ext));

        const validTypes = ["application/xml", "text/xml", "application/dmn20-xml"];
    ...
    async _handleFileImport(file) {
        ...
        try {
            ...
            console.error("Error importing DMN file:", err);
            this.notification.add(
                this.env._t(
                    "Error importing DMN file. Please make sure it is a valid DMN XML file."
                ),
                {type: "danger"}
            );
        }
    ...
    async onFileSelected(ev) {
        ...
        if (!this._validateFile(file)) {
            this.notification.add("Invalid file. Please select a .dmn or .xml file.", {
                type: "danger",
            });
            if (this.fileInputRef.el) {
     ...
     */
    async onExportClick() {
        if (this.dmnApi && this.dmnApi.exportDiagram) {
            const xmlContent = await this.dmnApi.exportDiagram();
            if (xmlContent) {
                this._downloadDmnFile(xmlContent);
            }
        } else {
            console.error("DMN modeler not ready yet");
        }
    }
    ...
    async onExportSVGClick() {
        if (this.dmnApi && this.dmnApi.exportSVG) {
            const svg = await this.dmnApi.exportSVG();
            if (svg) {
    ...
    async onExportPNGClick() {
        if (this.dmnApi && this.dmnApi.exportPNG) {
            const pngBlob = await this.dmnApi.exportPNG();
            if (pngBlob) {
    ...
    async onExportPDFClick() {
        if (this.dmnApi && this.dmnApi.exportPDF) {
            const pdfBlob = await this.dmnApi.exportPDF();
            if (pdfBlob) {
    ...
    _updateValidationStats() {
        if (this.dmnApi && this.dmnApi.getValidationStats) {
            const stats = this.dmnApi.getValidationStats();
            if (stats) {
                this.state.validationStats = stats;
            }
        }
     ...
     */
    onZoomInClick() {
        if (this.dmnApi && this.dmnApi.zoomIn) {
            this.dmnApi.zoomIn();
        }
    }
     ...
     */
    onZoomOutClick() {
        if (this.dmnApi && this.dmnApi.zoomOut) {
            this.dmnApi.zoomOut();
        }
    }
     ...
     */
    onZoomFitClick() {
        if (this.dmnApi && this.dmnApi.zoomFit) {
            this.dmnApi.zoomFit();
        }
    }
     ...
     */
    onZoomResetClick() {
        if (this.dmnApi && this.dmnApi.zoomReset) {
            this.dmnApi.zoomReset();
        }
    }

    /**
     * Downloads the DMN XML as a .dmn file.
     * @param {String} xmlContent The DMN XML string.
     * @private
     */
    _downloadDmnFile(xmlContent) {
        // Get diagram name from record or use default
        const diagramName = this.props.record?.data?.name || "diagram";
        const fileName = `${diagramName.replace(/[^a-z0-9]/gi, "_")}.dmn`;
...
registry.category("fields").add("dmn_modeler", DmnModelerWidget);
