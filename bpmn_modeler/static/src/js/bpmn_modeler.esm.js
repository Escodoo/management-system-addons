...
export class BPMNModeler extends Component {
    ...
    setup() {
        ...
        onMounted(() => {
            this._initBPMNModeler();
        });

        onWillUpdateProps((nextProps) => {
            if (nextProps.bpmn_xml !== this.props.bpmn_xml) {
                this._importXML(nextProps.bpmn_xml);
            }
        });
    ...
    /**
     * Loads the bpmn.js library CSS from CDN if not already loaded.
    ...
    async _loadBpmnJs() {
        return new Promise((resolve, reject) => {
            ...
            if (document.querySelector('script[src*="bpmn-modeler"]')) {
            ...
            script.onerror = () => {
                console.error("Failed to load BPMN.js modeler from CDN");
                reject(new Error("Failed to load BPMN.js modeler from CDN"));
            };
    ...
    /**
     * Loads BPMN.js CSS files from CDN
     * Order is important: diagram-js.css, bpmn-js.css, bpmn-font/css/bpmn.css
    ...
    /**
     * Initializes the bpmn.js modeler.
     * @private
     */
    async _initBPMNModeler() {
        try {
            // Ensure CSS is loaded first (critical for proper rendering)
            await this._loadBpmnCss();
            ...
            // Use BpmnJS from window (bpmn.js library)
            // Note: keyboard.bindTo was removed in newer versions of bpmn.js
