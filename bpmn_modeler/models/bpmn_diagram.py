...
class BpmnDiagram(models.Model):
    ...
    version_history_ids = fields.One2many(
        "bpmn.diagram.version", "diagram_id", string="Version History", readonly=True
    )
    ...
    template_id = fields.Many2one(
        "bpmn.template", string="Created from Template", readonly=True
    )

    # Category
    category_id = fields.Many2one(
        "bpmn.diagram.category", string="Category", tracking=True
    )
  ...
  <bpmndi:BPMNDiagram id="BPMNDiagram_1">
    ...
    def write(self, vals):
                ...
                if record.state != "draft":
                    ...
                    raise UserError(
                        _(
                            ...
                            "category, and BPMN content) when it is in 'Draft' "
        ...
        if "bpmn_xml" in vals and vals["bpmn_xml"]:
            # Get default value for bpmn_xml field
            field = self._fields["bpmn_xml"]
            ...
            for record in self:
                ...
                if (
                    record.bpmn_xml
                    and record.bpmn_xml != vals["bpmn_xml"]
                    and record.bpmn_xml != default_xml
                ):
